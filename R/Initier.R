...
### variable name. 
### It assumes that all factors are present in the data as K-1 new variables with appropriate contrast value as numeric variables. 
...
Initer <- R6::R6Class(
  ...
  inherit = Scaffold,
  public=list(
    vars       =  NULL,    # all selected variables
    ok         =  TRUE,    # is everything ok to go 
    datamatic  =   NULL,   # data manipulation object
    structure  =   NULL,   # self$structure contains all parameters with plain names
    model      =   NULL,   # contains the estimated model
    ...
    initialize=function(jmvobj) {
                ...
                self$vars      = unlist(c(jmvobj$options$endogenous,jmvobj$options$factors,jmvobj$options$covs))
                self$datamatic = Datamatic$new(jmvobj)
                self$contrasts_names<-self$datamatic$contrasts_names
                self$multigroup=self$datamatic$multigroup
  ...
  private=list(
            ...
            .check_interactions=function() {
                            ...
                            if (is.something(res)) {
                                    w<-paste(WARNS[["nocenterd"]],paste(res,collapse = ", "))
                                   self$warning<-list(topic="main",message=w)
                            }
